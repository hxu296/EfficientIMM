...
void static dma_bcast1_add_bitmap(LP sgl, T val, std::size_t n, BP base) {
    #pragma unroll
...
template<typename LP, typename T, typename BP>
void static dma_bcast1_add_index(LP sgl, T val, std::size_t n, BP base) {
    #pragma unroll
    for (auto i = 0ul; i < n; ++i){
        #pragma omp atomic 
        base[sgl[i]] = base[sgl[i]] + val;
    }
}
...
auto FindMostInfluentialSet_atomics(const GraphTy &G, long* counter, long theta, long k,
                            const std::vector<AdaptiveRRRType>& ref_RRRsets) {
    using vertex_type = typename GraphTy::vertex_type;

    std::cout<<"Atomics Adaptive RRRSet version"<<std::endl;
...
#pragma omp single
            ...
            if(update_inc) {
                ...
                for (int it_theta = theta_strt; it_theta < theta_end; ++it_theta) {
                    if (loc_covered[it_theta] == true) continue;

                    if (auto p = std::get_if<std::unique_ptr<std::vector<uint64_t>>>(&ref_RRRsets[it_theta])) {
                        // get the RRRset size
                        long cur_set_size = (*p)->size();
                        if (!binarySearch(**p, cur_set_size, max_vert)) {
                        dma_bcast1_add_index(**p, 1l, cur_set_size, cur_counter);
                        }else{
                            loc_covered[it_theta] = true;
                        }
                    } else if (auto p = std::get_if<std::unique_ptr<std::vector<bool>>>(&ref_RRRsets[it_theta])) {
                        long cur_set_size = (*p)->size();
                        if (!(**p)[max_vert]) {
                        dma_bcast1_add_bitmap(**p, 1l, cur_set_size, cur_counter);
                        }else{
                            loc_covered[it_theta] = true;
                        }
                    }
